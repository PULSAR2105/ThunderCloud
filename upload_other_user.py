...
#To send a file from another user :
def query_upload_other_user(client, infos_client) :
    #Opening of log file :
...
def upload_other_user() :
    ...
    server.listen(5)
    log = open("logs/download_other_user.txt", "a")
    log.write(time.strftime(">> [%d/%m/%y - %H:%M] The server 'upload_other_user' listen on the port : " + str(port) + "\n"))
    log.close()
    #--------------------
